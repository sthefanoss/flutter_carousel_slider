    ...
    return Scaffold(
      ...
      body: ListView(
        children: <Widget>[
          ...
          DemoItem('Callbacks demo', '/callbacks'),
...
class CallbacksDemo extends StatefulWidget {
  @override
  _CallbacksDemoState createState() => _CallbacksDemoState();
}

class _CallbacksDemoState extends State<CallbacksDemo> {
  String _log = "";

  void _addToLog(String input) => setState(() => _log = '$input\n$_log');

  @override
  Widget build(BuildContext context) {
    List<int> list = [1, 2, 3, 4, 5];
    return Scaffold(
      appBar: AppBar(title: Text('Callbacks demo')),
      body: Column(
        children: [
          Container(
              child: CarouselSlider(
            options: CarouselOptions(
              onDragStart: (details) => _addToLog(details.toString()),
              onDragDown: (details) => _addToLog(details.toString()),
              onDragUpdate: (details) => _addToLog(details.toString()),
              onDragEnd: (details) => _addToLog(details.toString()),
              onDragCancel: () => _addToLog('DragCancel()'),
            ),
            items: list
                .map((item) => Container(
                      child: Center(child: Text(item.toString())),
